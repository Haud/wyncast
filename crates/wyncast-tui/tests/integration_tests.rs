...
use wyncast_tui::app::{self, AppState};
use wyncast_tui::config::*;
use wyncast_tui::db::Database;
use wyncast_tui::draft::pick::{DraftPick, Position};
use wyncast_tui::draft::state::DraftState;
use wyncast_tui::llm::client::LlmClient;
use wyncast_tui::llm::prompt::BudgetContext;
use wyncast_tui::protocol::*;
use wyncast_tui::valuation::projections::{AllProjections, PitcherType};
use wyncast_tui::valuation::zscore::PlayerValuation;
use wyncast_tui::stats::StatRegistry;
use wyncast_tui::ws_server::WsEvent;
...
fn ten_team_budgets() -> Vec<wyncast_tui::draft::state::TeamBudgetPayload> {
    (1..=10)
        .map(|i| wyncast_tui::draft::state::TeamBudgetPayload {
            team_id: format!("{}", i),
...
fn load_fixture_players(config: &Config) -> Vec<PlayerValuation> {
    let projections = wyncast_tui::valuation::projections::load_all_from_paths(
        &config.data_paths,
    )
    ...
    wyncast_tui::valuation::compute_initial(&projections, config, &roster_config(), &registry)
        .expect("initial valuation should succeed")
}

/// Load fixture projections.
fn load_fixture_projections(config: &Config) -> AllProjections {
    wyncast_tui::valuation::projections::load_all_from_paths(
        &config.data_paths,
    )
    .expect("fixture CSVs should load")
    .expect("fixture CSV paths are configured")
...
fn create_test_app_state_from_fixtures() -> AppState {
    ...
    let registry = StatRegistry::from_league_config(&config.league)
        .expect("test registry");
    wyncast_tui::valuation::recalculate_all(
        &mut available,
        &roster,
        &config.league,
        &config.strategy,
    ...
    let onboarding_manager = wyncast_tui::onboarding::OnboardingManager::new(
...
fn nomination_analysis_prompt_contains_required_sections() {
    ...
    let prompt = wyncast_tui::llm::prompt::build_nomination_analysis_prompt(
        &player,
        &nomination,
...
fn nomination_planning_prompt_contains_required_sections() {
    ...
    let prompt = wyncast_tui::llm::prompt::build_nomination_planning_prompt(
        &state.draft_state.my_team().unwrap().roster,
        &state.category_needs,
        &state.scarcity,
        &state.available_players,
...
fn alt_league_config_full_pipeline() {
    ...
    let players = wyncast_tui::valuation::compute_initial(
        &projections,
        &config,
        &roster_config(),
        &registry,
