...
impl Database {
    ...
    /// linkage is deferred). The `espn_player_id` is the external ESPN
    /// identifier from the extension. Timestamp is auto-generated by SQLite.
    pub fn record_pick(&self, pick: &DraftPick, espn_player_id: Option<&str>) -> Result<()> {
        let conn = self.conn.lock().expect("database mutex poisoned");
        conn.execute(
            "INSERT INTO draft_picks (pick_number, team_id, team_name, player_id, espn_player_id, player_name, position, price)
    ...
    pub fn load_picks(&self) -> Result<Vec<DraftPick>> {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT pick_number, team_id, team_name, player_name, position, price
    ...
    pub fn upsert_player(
        ...
        // Always SELECT the id rather than relying on last_insert_rowid(),
        // which returns 0 when ON CONFLICT triggers an UPDATE.
        let id: i64 = conn
            .query_row(
            ...
            .context("failed to look up player id after upsert")?;
        Ok(id)
    }
...
mod tests {
    ...
    fn insert_and_load_picks_round_trip() {
        ...
        db.record_pick(&pick1, Some("espn_1")).unwrap();
        db.record_pick(&pick2, Some("espn_2")).unwrap();
    ...
    fn record_pick_stores_espn_player_id() {
        let db = test_db();
        db.record_pick(&sample_pick(1), Some("12345")).unwrap();
        db.record_pick(&sample_pick(2), None).unwrap();

        let conn = db.conn.lock().unwrap();
        let espn_id: Option<String> = conn
            .query_row(
                "SELECT espn_player_id FROM draft_picks WHERE pick_number = 1",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(espn_id, Some("12345".to_string()));

        let espn_id_none: Option<String> = conn
            .query_row(
                "SELECT espn_player_id FROM draft_picks WHERE pick_number = 2",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert!(espn_id_none.is_none());
    }

    #[test]
    fn record_pick_auto_generates_timestamp() {
        let db = test_db();
        db.record_pick(&sample_pick(1), None).unwrap();
